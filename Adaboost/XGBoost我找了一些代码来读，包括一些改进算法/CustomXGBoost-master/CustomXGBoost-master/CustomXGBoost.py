...
class Node:
    ...
    def predict_single_val(self, x):
        ...
        return self.lhs_tree.predict_single_val(x) if x[self.split_col] <= self.pivot else self.rhs_tree.predict_single_val(
...
class XGBRegressor:  # base xgb
    def __init__(self, eta=0.3, n_estimators=100, max_depth=6, gamma=0, min_child_weight=1, lambda_=1, colsample=1,
                 subsample=1):
        self.eta = eta
...
class XGBClassifier:
    def __init__(self, n_classes, eta=0.3, n_estimators=100, max_depth=6, gamma=0, min_child_weight=1, lambda_=1,
                 colsample=1, subsample=1):
